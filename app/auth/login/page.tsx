        ...
        <div className="w-full max-w-sm mx-auto">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isPending}
              className="w-full bg-primary text-white rounded-lg py-2.5 font-medium hover:bg-primary-hover transition-colors flex justify-center items-center gap-2 disabled:opacity-70"
            >
              {isPending ? (
                <>
          ...
          <p className="mt-8 text-center text-sm text-gray-600">
            Não tem uma conta?{' '}
            <Link href={`/auth/cadastro${redirectPath ? `?redirect=${encodeURIComponent(redirectPath)}` : ''}`} className="text-primary font-medium hover:underline">
              Cadastre-se
            </Link>
          </p>
        </div>
